...
“延迟加载”（也叫 “懒加载”）的本质是 “不提前做准备，等需要的时候再做”。`Lazy<T>` 就是帮我们自动实现这个逻辑，避免 “提前创建用不到的对象” 造成的资源浪费。

举个直观例子：

* 不使用 `Lazy<T>`：类初始化时就创建 `HeavyObject`（假设它创建耗时、占内存），哪怕后续根本用不到。
* 使用 `Lazy<T>`：只有第一次访问 `HeavyObject` 时，才会创建它，用不到就不创建，节省资源。

### 二、`Lazy<T>` 的 3 个核心价值

#### 1. 优化程序启动速度

如果一个类中有很多 “创建耗时” 的对象（比如读取配置、初始化大集合、连接数据库等），不使用 `Lazy<T>` 会导致类初始化时 “一次性加载所有对象”，程序启动变慢。

用 `Lazy<T>` 后，这些对象只在第一次使用时创建，启动时仅加载 “轻量” 的 `Lazy<T>` 包装器，启动速度大幅提升。

#### 2. 节省内存资源

很多对象可能在程序运行中 “根本用不到”（比如某些功能是用户可选的）。

* 不使用 `Lazy<T>`：不管用不用，对象都会被创建并占用内存。
* 使用 `Lazy<T>`：不用就不创建，直接节省这部分内存。

#### 3. 避免不必要的依赖初始化

如果一个对象的创建依赖其他资源（比如网络、文件），提前创建可能导致依赖未准备好（如网络未连接），或造成不必要的依赖加载。

`Lazy<T>` 能确保 “在使用时才去初始化依赖”，避免提前加载的风险。

### 三、`Lazy<T>` 的基本用法

`Lazy<T>` 是 C# 中实现 “延迟加载” 的核心工具，它的核心作用是 **让对象在 “第一次被使用时才创建”**，而非程序启动或类初始化时就创建，从而优化资源占用和程序性能。
...
public class HeavyObject
{
    ...
    public void DoWork() => Console.WriteLine("HeavyObject 正在工作...");
}
```
...
* 调用 `UseHeavyObject()` 前，`HeavyObject` 不会创建；
* 第一次调用 `UseHeavyObject()` 时，才会输出 “开始创建...”→“创建完成！”→“正在工作...”；
* 后续再访问 `_lazyHeavyObj.Value`，直接返回已创建的对象，不再重复创建。
