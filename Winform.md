...
setting中存放的数据的是简单的字符串，键值对。setting是单例模式，需要先获取setting才能获取数据。
...
为什么一个单例模式一个全局静态？一句话概括：  **是 “活的、可改的配置，可读可写全局一个对象”，需要单例管理状态；是 “死的、只读的资源”，静态访问最简洁高效** 。
